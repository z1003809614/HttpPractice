...
namespace myhttp
{
                    ...
                    int family, int type, int protocol)
    {
        ...
        // 根据node(ipv6 or ipv4 or domain)和service来获得addr信息；
        int error = getaddrinfo(node.c_str(), service, &hints, &results);
        if(error){
            MYHTTP_LOG_ERROR(g_logger) << "Address::Lookup getaddress(" << host << ","
                    ...
                    int family)
    {
        ...
        if(getifaddrs(&results) != 0){
            MYHTTP_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
        ...
        try{
            ...
            MYHTTP_LOG_ERROR(g_logger) << "Address::GetInterfaceAddress exception";
            freeifaddrs(results);
            return false;
        }
        // 释放该结构体；
    ...
    IPAddress::ptr IPAddress::Create(const char* address, uint16_t port){
        ...
        // hits主要是起过滤的作用；
        hints.ai_flags = AI_NUMERICHOST; // 只能使用数字address，而不能是域名；
        hints.ai_family = AF_UNSPEC;

        int error = getaddrinfo(address, NULL, &hints, &results);
        if(error){
            MYHTTP_LOG_ERROR(g_logger) << "IPAddress::Create(" << address
                << ", " << port << ") error=" << error
...
// ================================IPv4Address=====================================
    IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port){
        ...
        if(result <= 0){
            MYHTTP_LOG_ERROR(g_logger) << "IPv4Address::Create(" << address << ", "
...
// ================================IPv6Address=====================================
    ...
    IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port){
        ...
        if(result <= 0){
            MYHTTP_LOG_ERROR(g_logger) << "IPv6Address::Create(" << address << ", "
                << port << ") rt=" << result << " errno=" << errno
                << " errstr=" << strerror(errno);
            return nullptr;
        }
